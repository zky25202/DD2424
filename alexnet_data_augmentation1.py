...
train_data2 = dset.CIFAR10(root='./data_augmentation', train=True, transform=transform2, download=False)
test_data2 = dset.CIFAR10(root='./data_augmentation', train=False, transform=transform2, download=False)
#train_loader2 = torch.utils.data.DataLoader(train_data, batch_size=batch_size, shuffle=True)
test_loader2 = torch.utils.data.DataLoader(test_data, batch_size=batch_size, shuffle=True)

train_loader2 = torch.utils.data.DataLoader(dataset =train_data,
                                                batch_size = batch_size,
                                                sampler = sampler_train
                                               )
validation_loader2 = torch.utils.data.DataLoader(dataset=train_data,
                                          batch_size=batch_size,
                                          sampler = sampler_val
                                         )
...
for epoch in range(5):
    ...
    data augmentation 
    ''' 
    for batch in validation_loader2:
        images, labels = batch
        outs = model(images.to(device))
        loss = F.cross_entropy(outs, labels.to(device))
        total_loss_val += loss.item()
